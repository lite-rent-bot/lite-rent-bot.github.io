...
const Form = () => {
    ...
    const onScooter1Scanned = async (tg_event) => {
		await setScooter("тест:"+JSON.stringify(tg_event));
		let num = tg_event.slice(-5).match(/\d+/)[0];
		//let isnum = /^\d+$/.test(num);
		if (num.length==4||num.length==5) {
			...
			//	setScooter(num);
			//console.log("scooter:"+JSON.stringify(scooter));
			//await setScooter("ошибка4:"+JSON.stringify(tg_event));
		}		
    ...
    return (
	   <div className={"form"}>
		   ...
		   <h3>Режим сканирования по 1 самокату:</h3>			
            ...
            />
			<Button class="simplebutton" onClick={onScooter1ScannerClick}>Сканировать 1 самокат</Button>			
        </div>
    );
};
